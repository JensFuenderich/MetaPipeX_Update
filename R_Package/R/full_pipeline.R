...
full_pipeline <- function(data, MultiLab = NULL, ReplicationProject = NULL, Replication = NULL, DV = NULL, Group = NULL, output_path = NULL, folder_name = NULL, suppress_list_output = FALSE, method = "REML", sparse = FALSE){
  ...
  if (is.null(output_path) == TRUE) {
    output_list$Replication_Summaries <- MetaPipeX_Update::create_replication_summaries(data = data,
                                                                                MultiLab = {{MultiLab}},
                                                                                ReplicationProject = {{ReplicationProject}},
                                                                                Replication = {{Replication}},
                                                                                DV = {{DV}},
  ...
  if (is.null(output_path) == TRUE) {
    output_list$Merged_Replication_Summaries <- MetaPipeX_Update::merge_replication_summaries(data = output_list$Replication_Summaries$Replication_Summaries,
                                                                                      suppress_list_output = FALSE)
  } else {
    output_list$Merged_Replication_Summaries <- MetaPipeX_Update::merge_replication_summaries(data = output_list$Replication_Summaries$Replication_Summaries,
  ...
  #### ATTENTION: The following code chunk kills small labs, because the meta-analyses of SD seem to have issues with negative sampling variances!
  ## Probably needs a better solution

  output_list$Merged_Replication_Summaries$Merged_Replication_Summaries <- subset(output_list$Merged_Replication_Summaries$Merged_Replication_Summaries, C_N > 2 & T_N > 2)


  if (is.null(output_path) == TRUE) {
    output_list$Meta_Analyses <- MetaPipeX_Update::meta_analyses(data = output_list$Merged_Replication_Summaries$Merged_Replication_Summaries,
                                                         suppress_list_output = FALSE,
                                                         method = method,
                                                         sparse = sparse)
  } else {
