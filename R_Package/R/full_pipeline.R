...
#' The yellow blocks with rounded corners are .csv files. The purple/white rectangles each refer to a step in the pipeline that is performed by a MetaPipeX function. MetaPipeXUpdate::full_pipeline() performs the steps 3-6 and returns "meta_pipe_x_data.csv" which may be provided to the MetaPipeX App for handy data selection and basic plotting of the analysis results.
...
full_pipeline <- function(data, MultiLab = NULL, MASC = NULL, Data_Collection_Site = NULL, DV = NULL, Group = NULL, output_path = NULL, folder_name = NULL, suppress_list_output = FALSE, method = "REML", sparse = FALSE){
  ...
  codebook_for_individual_participant_data <- MetaPipeXUpdate:::create_IPD_codebook()
  ...
  if (is.null(output_path) == TRUE) {
    output_list$Site_Summaries <- MetaPipeXUpdate::summarize_sites(data = data,
                                                             MultiLab = {{MultiLab}},
                                                             MASC = {{MASC}},
                                                             Data_Collection_Site = {{Data_Collection_Site}},
                                                             DV = {{DV}},
  ...
  if (is.null(output_path)) {
    output_list$Merged_Site_Summaries <- MetaPipeXUpdate::merge_site_summaries(data = output_list$Site_Summaries$Site_Summaries,
                                                                         suppress_list_output = FALSE)
  } else {
    output_list$Merged_Site_Summaries <- MetaPipeXUpdate::merge_site_summaries(data = output_list$Site_Summaries$Site_Summaries,
  ...
  if (is.null(output_path)) {
    output_list$Meta_Analyses <- MetaPipeXUpdate::meta_analyze_MASCs(data = output_list$Merged_Site_Summaries$Merged_Site_Summaries,
                                                               suppress_list_output = FALSE,
                                                               method = method,
                                                               sparse = sparse)
  } else {
  ...
  if (is.null(output_path)) {
    output_list$Meta_Pipe_X <- MetaPipeXUpdate::create_MetaPipeX_format(Merged_Site_Summaries = output_list$Merged_Site_Summaries$Merged_Site_Summaries,
