...
merge_site_summaries <- function(data, output_folder = NULL, suppress_list_output = FALSE){
  ...
  if (is.list(data) != TRUE && file.exists(data) != TRUE) {
    ...
    codebook_for_merged_site_summeries <- MetaPipeXUpdate:::create_site_summary_codebook(description_vector = names(merged_site_summaries))
