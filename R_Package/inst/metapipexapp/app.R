...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    mainPanel(
      ...
      conditionalPanel(condition = "input.select_upload == 'SiteSum'",
                       h3("Site Level Data"),
                       h5("Please provide at least one .csv that has been produced by MetaPipeXUpdate::summarize_sites() or is arranged according to the", tags$a(href="https://github.com/JensFuenderich/MetaPipeX/blob/main/Supplementary_Material/Table_Templates/lvl2_site_summaries/site_summaries_template.csv", "template on github.")),
      ...
      conditionalPanel(condition = "input.select_upload == 'MergedSiteSum'",
                       ...
                       h5("Please provide a single .csv that has been produced by MetaPipeXUpdate::merge_site_summaries() or is arranged according to the", tags$a(href="https://github.com/JensFuenderich/MetaPipeX/blob/main/Supplementary_Material/Table_Templates/lvl3_merged_site_summaries/merged_site_summaries_template.csv", "template on github.")),
      ...
      conditionalPanel(condition = "input.select_upload == 'MetaPipeX'",
                       h3("MetaPipeX Data"),
                       h5("Please provide a single .csv that has been produced by MetaPipeXUpdate::full_pipeline() or is arranged according to the", tags$a(href="https://github.com/JensFuenderich/MetaPipeX/blob/main/Supplementary_Material/Table_Templates/lvl5_meta_pipe_x/meta_pipe_x_data_template.csv", "template on github.")),
...
server <- function(input, output, session){
    ...
    if (input$select_upload == "SimulateData") {
      ...
      withProgress(message = 'Calculation in progress. This may take a moment.',
                   ...
                   style = "old",
                   {
                     ...
                     MetaPipeX_output <- MetaPipeXUpdate::full_pipeline(data = SiteSum_list)
  ...
  observeEvent(input$confirm_upload,{ # stores results in data_import$SiteSum_MetaPipeX
    ...
    if (input$select_upload == "SiteSum") {
      ...
      withProgress(message = 'Calculation in progress. This may take a moment.',
                   ...
                   {
                     # merge the Site summaries
                     MergedSiteSum <- MetaPipeXUpdate::merge_site_summaries(data = SiteSum_list)

                     # run meta analyses
                     Meta_Analyses <- MetaPipeXUpdate::meta_analyze_MASCs(data = MergedSiteSum$Merged_Site_Summaries)

                     ## combine Site and meta analysis data
                     MetaPipeX_output <- MetaPipeXUpdate::create_MetaPipeX_format(
  ...
  observeEvent(input$confirm_upload,{ # stores results in data_import$MergedSiteSum_MetaPipeX
    ...
    if (input$select_upload == "MergedSiteSum") {
      ...
      withProgress(message = 'Calculation in progress. This may take a moment.',
                   ...
                   {
                     # run meta analyses
                     Meta_Analyses <- MetaPipeXUpdate::meta_analyze_MASCs(data = MergedSiteSum)
  ...
  output$zip_download <- downloadHandler(
    ...
    content = function(file){
      ...
      if (input$select_upload == "IPD") {
        ...
        utils::write.csv(data_import$transformations, file = file.path("MetaPipeX_folder/0_Input/transform_to_IPD.csv"))
        # save codebook for transformations
        utils::write.csv(data_import$codebook_transformations, file = file.path("MetaPipeX_folder/0_Input/codebook_for_transform_to_IPD.csv"))
        ...
        utils::download.file("https://raw.githubusercontent.com/JensFuenderich/MetaPipeX/main/Supplementary_Material/Analysis_Documentation/MetaPipeX_Analysis_Documentation.R",
                             file.path("MetaPipeX_folder/0_Input/MetaPipeX_Analysis_Documentation.R"))
      } else if (input$select_upload == "SimulateData") {
        # save simulation function with input
        utils::write.table(data.frame(function_to_recreate_simulation = paste("# remove quotations and execute the function: mapply(MetaPipeXUpdate::simulate_IPD, MASC_index = 1:", input$number_of_MASCs, ", seed = ", input$seed, " + (0:(", input$number_of_MASCs, "-1)), SIMPLIFY = FALSE)", sep = "")),
                         file = file.path("MetaPipeX_folder/0_Input/SimData_Functions.R"))
        # save data as simulated
        utils::write.csv(data_import$SimData_Input, file = file.path("MetaPipeX_folder/0_Input/SimData_Input.csv"))
      }
      ...
      utils::write.csv(data_import$IPD_data$lvl1_individual_participant_data$codebook_for_individual_participant_data, file.path(paste("MetaPipeX_folder/lvl1_individual_participant_data/codebook_for_individual_participant_data.csv", sep = "")))
      ...
      dir.create(file.path(paste("MetaPipeX_folder", "/lvl2_site_summaries", sep = "")))
      ...
      dir.create(file.path(paste("MetaPipeX_folder", "/lvl3_merged_site_summaries", sep = "")))
      ...
      utils::write.csv(data_import$IPD_data$lvl5_meta_pipe_x$codebook_for_meta_pipe_x,
  ...
  observe({
    ...
    if (stringr::str_detect(as.character(input$kernel_density_est_data_est), "_C_M")) {
      updateVarSelectInput(session, "kernel_density_est_data_model_est",
                           data = data(),
                           selected = "MA__Est__C_M")
      updateVarSelectInput(session, "kernel_density_est_data_Tau",
                           data = data(),
                           selected = "MA__Tau__C_M")
    } else if (stringr::str_detect(as.character(input$kernel_density_est_data_est), "_T_M")) {
      updateVarSelectInput(session, "kernel_density_est_data_model_est",
                           data = data(),
                           selected = "MA__Est__T_M")
      updateVarSelectInput(session, "kernel_density_est_data_Tau",
                           data = data(),
                           selected = "MA__Tau__T_M")
    } else if (stringr::str_detect(as.character(input$kernel_density_est_data_est), "_C_SD")) {
      updateVarSelectInput(session, "kernel_density_est_data_model_est",
                           data = data(),
                           selected = "MA__Est__C_SD")
      updateVarSelectInput(session, "kernel_density_est_data_Tau",
                           data = data(),
                           selected = "MA__Tau__C_SD")
    } else if (stringr::str_detect(as.character(input$kernel_density_est_data_est), "_T_SD")) {
      updateVarSelectInput(session, "kernel_density_est_data_model_est",
                           data = data(),
                           selected = "MA__Est__T_SD")
      updateVarSelectInput(session, "kernel_density_est_data_Tau",
                           data = data(),
                           selected = "MA__Tau__T_SD")
    } else if (stringr::str_detect(as.character(input$kernel_density_est_data_est), "_pooled_SD")) {
      updateVarSelectInput(session, "kernel_density_est_data_model_est",
                           data = data(),
                           selected = "MA__Est__pooled_SD")
      updateVarSelectInput(session, "kernel_density_est_data_Tau",
                           data = data(),
                           selected = "MA__Tau__pooled_SD")
    ...
    } else if (stringr::str_detect(as.character(input$kernel_density_est_data_est), "_SMD")) {
      updateVarSelectInput(session, "kernel_density_est_data_model_est",
                           data = data(),
  ...
  observe({
    ...
    if (stringr::str_detect(as.character(input$forest_data_statistics), "_C_M")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_C_M")
    } else if (stringr::str_detect(as.character(input$forest_data_statistics), "_T_M")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_T_M")
    } else if (stringr::str_detect(as.character(input$forest_data_statistics), "_C_SD")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_C_SD")
    } else if (stringr::str_detect(as.character(input$forest_data_statistics), "_T_SD")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_T_SD")
    } else if (stringr::str_detect(as.character(input$forest_data_statistics), "_pooled_SD")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_pooled_SD")
    } else if (stringr::str_detect(as.character(input$forest_data_statistics), "_MD")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_MD")
    } else if (stringr::str_detect(as.character(input$forest_data_statistics), "_SMD")) {
      updateVarSelectInput(session, "forest_data_SE",
                           data = data(),
  ...
  observe({
    ...
    if (stringr::str_detect(as.character(input$funnel_data_est), "_C_M")) {
      ...
      updateVarSelectInput(session, "funnel_data_model_est",
                           data = data(),
                           selected = "MA__Est__C_M")
    } else if (stringr::str_detect(as.character(input$funnel_data_est), "_T_M")) {
      updateVarSelectInput(session, "funnel_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_T_M")
      updateVarSelectInput(session, "funnel_data_model_est",
                           data = data(),
                           selected = "MA__Est__T_M")
    } else if (stringr::str_detect(as.character(input$funnel_data_est), "_C_SD")) {
      updateVarSelectInput(session, "funnel_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_C_SD")
      updateVarSelectInput(session, "funnel_data_model_est",
                           data = data(),
                           selected = "MA__Est__C_SD")
    } else if (stringr::str_detect(as.character(input$funnel_data_est), "_T_SD")) {
      updateVarSelectInput(session, "funnel_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_T_SD")
      updateVarSelectInput(session, "funnel_data_model_est",
                           data = data(),
                           selected = "MA__Est__T_SD")
    } else if (stringr::str_detect(as.character(input$funnel_data_est), "_pooled_SD")) {
      updateVarSelectInput(session, "funnel_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_pooled_SD")
      updateVarSelectInput(session, "funnel_data_model_est",
      ...
      updateVarSelectInput(session, "funnel_data_model_est",
                           data = data(),
                           selected = "MA__Est__MD")
    } else if (stringr::str_detect(as.character(input$funnel_data_est), "_SMD")) {
      updateVarSelectInput(session, "funnel_data_SE",
                           data = data(),
                           selected = "Site__Summaries__SE_SMD")
      updateVarSelectInput(session, "funnel_data_model_est",
